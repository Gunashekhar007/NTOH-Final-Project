...
// Physical parameters (Tohoku 2011 scale)
const double g = 9.81;       // Gravity [m/s²]
const double ho = 2000.0;    // Deep ocean depth [m]
const double fbo = 0.003;    // Bed friction coefficient
const double zbmax = 5.0;    // Max seabed uplift [m]
const double nd = 30.0;      // Seabed motion duration [s]

// Numerical parameters
const double Dx = 500.0;     // Spatial step [m] (500m resolution)
const double Dt = 0.5;       // Time step [s] (CFL-stable)
...
int main() {
    ...
    const double c = sqrt(g * ho);       // Wave speed ~140 m/s
    ...
    const int radius = 20;               // ~10 km radius

    prepare_output_dir("wave_data");
